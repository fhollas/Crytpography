...
def judge_qr(y, p, q):
    if pow(y, int((q - 1) / 2), q) == 1 and pow(y, int((p - 1) / 2), p) == 1:
...
def choose_qnr(N, p, q):
    ...
    for y in range(N):
        ...
        #print(pow(y, int((q - 1) / 2)), q))
        #print(pow(y, int((p - 1) / 2)), p))
...
def bin2str(bin_str):
    ...
    # added cipher
    plaintxt2 = input("- Input added plain text :")
    # bin_str = bin(int(binascii.hexlify(plaintxt), 16))
